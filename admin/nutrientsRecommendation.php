...
if (isset($_POST['method']) && isset($_POST['id']) && $_POST['method'] == 'edit') {
    $updateSql = "UPDATE nutrients_recom SET `title` = '{$_POST['title']}' ,`text` = '{$_POST['text']}'";
    ...
    if (isset($_FILES['img']) && $_FILES['img']) {
        ...
        if ($file_size > 2097152) {
            header("Location: nutrientsRecommendation.php?error=The image size must be less than 2MB");
        }
        if (move_uploaded_file($file_tmp, "../uploads/nutrientsRecommendation/" . $file_name)) {
            $imagePath = "uploads/nutrientsRecommendation/" . $file_name;
    ...
    runQuery($updateSql);
    header("Location: nutrientsRecommendation.php");
}
...
if (isset($_POST['method']) && $_POST['method'] == 'new') {
    ...
    $insertSql = "INSERT INTO `nutrients_recom`(`title`, `text`, `img`) VALUES ('{$_POST['title']}','{$_POST['text']}','{$imagePath}')";
    runQuery($insertSql);
    header("Location: nutrientsRecommendation.php");
}
...
if (isset($_GET['method']) && $_GET['method'] == 'DELETE' && isset($_GET['id'])) {
    $deleteID = $_GET['id'];
    $sql = "DELETE FROM nutrients_recom WHERE id = '{$deleteID}'";
    runQuery($sql);
    header('Location: nutrientsRecommendation.php');
}
...
<head>
    <meta charset="utf-8"/>
    <title>Nutrients Recommendation</title>
    <?php
